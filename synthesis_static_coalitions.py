...
def _strategy_given_rank(rank, product_game, conc_game, values, n_players, ranks,coalition):
    ...
    while True:
        d = pre(states, conc_game,coalition)
        set_u = d.keys() - states
            ...
            for players, act in d[u]:
                ...
                if players != 1:
                    ...
                    if values[u][player_i - 1] < max(costs[v][player_i - 2] for v in next_states_under_a):
                        ...
                        # This 'continue' is for if the joint action is not rationalizable, the rest of the iteration does not have to be done.
                        continue
                        # TODO
...
def get_coalition_actions(game, u,coalition):
    ...
    for _, _, act, _ in game.transitions(from_state=u):
        for players in coalition:
            if players !=1:
                _, player_i = players
                # Update enabled actions dictionary
                en_actions.add((players, (act[0], act[player_i - 1])))
            else:
...
def pre(set_u, conc_game, coalition):
    frontier = dict()

    for state in conc_game.predecessors(set_u):
        eliminated_actions = set()
        for (players, act_i) in get_coalition_actions(conc_game, state,coalition):
            for _, next_state, action, _ in conc_game.transitions(from_state=state):
                if players != 1:
                    ...
                    if act_i == action[0] and next_state not in set_u:
                            eliminated_actions.add((players, act_i))
        if get_coalition_actions(conc_game, state,coalition) != eliminated_actions:
            frontier[state] = get_coalition_actions(conc_game, state,coalition) - eliminated_actions
...
def synthesis(product_game, conc_game, ranks, values, n_players, coalition):
    # Compute max rank
    ...
    rank=0
    win_states, general_costs = _strategy_given_rank(rank, product_game, conc_game, values, n_players, ranks,coalition)
    # print("kaan")
...
if __name__ == '__main__':
    ...
    s,gen = synthesis(product_game, conc_game, ranks_conc_game, costs, 3,{(1,3)})
